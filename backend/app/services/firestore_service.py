...
class FirestoreService:
    ...
    async def get_logs(
        ...
        logs = [{'id': doc.id, **doc.to_dict()} for doc in docs[:limit]]

        return {"logs": logs, "hasMore": has_more}
    ...
    async def create_policy(self, policy_data: dict) -> str:
        ...
        doc_ref = self.db.collection('policies').document()
        policy_data.update({
            'version': 1,
    ...
    async def get_active_policies_by_tenant(self, tenant_id: str) -> List[Dict]:
        ...
        docs = self.db.collection('policies')\
