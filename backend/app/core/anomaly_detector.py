...
class AnomalyDetector:
    ...
    async def _check_frequency_anomaly(self, history: List[Dict]) -> int:
        ...
        avg_requests_per_hour = sum(hourly_counts.values()) / len(hourly_counts)
        current_hour = datetime.utcnow().replace(minute=0, second=0, microsecond=0)
        current_count = hourly_counts.get(current_hour, 0)

        if current_count > (avg_requests_per_hour * 3):
            return 20
        elif current_count > (avg_requests_per_hour * 2):
            return 10
    ...
    async def _check_violation_history(self, history: List[Dict]) -> int:
        ...
        block_count = sum(1 for log in history if log.get('decision') == 'block')
