...
allowed_origins = os.getenv("ALLOWED_ORIGINS", "http://localhost:3000,https://prompt-firewall-frontend-390735445343.us-central1.run.app").split(",")
app.add_middleware(
    CORSMiddleware,
...
async def create_first_admin(request: AdminCreationRequest):
    admin_secret = os.getenv("ADMIN_CREATION_SECRET", "change-this-secret-in-production")
...
async def process_query(request: QueryRequest):
    ...
    try:
        ...
        prompt_decision = await firewall.analyze_request(
        ...
        prompt_to_send = (
            prompt_decision.promptModified
            if prompt_decision.decision == "redact"
            else request.prompt
        )
        ...
        response_decision = await firewall.analyze_request(
            prompt=request.prompt,
        ...
        final_response = (
            response_decision.responseModified
            if response_decision.decision in ["block", "redact"]
            else llm_response
        )
...
async def update_policy(
    ...
    await db_service.update_policy(policy_id, policy.model_dump(), user.get('email', 'admin'))
...
async def get_policy_history(
    ...
    try:
        ...
        error_client.report_exception()
        raise HTTPException(status_code=500, detail="Failed to retrieve policy history")
...
async def rollback_policy(
    ...
    try:
        ...
        await db_service.rollback_policy(policy_id, target_version)
        return {"status": "rolled_back", "message": f"Policy rolled back to version {target_version}"}
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except Exception as e:
        error_client.report_exception()
...
async def get_logs(
    ...
    tenant_id = user.get('tenantId', 'default')
    result = await db_service.get_logs(
        limit=limit,
        offset=offset,
